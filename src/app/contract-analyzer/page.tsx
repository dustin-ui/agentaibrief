...
const OFFER_LABELS = Array.from({ length: 20 }, (_, i) => String(i + 1));
      ...
      if (offerCount === 1) {
        ...
        if (!res.ok) {
          ...
          try {
            ...
            throw new Error(errText.slice(0, 200) || 'Analysis failed');
          }
        }
        ...
        if (compareResult && offersToAnalyze.length > 0 && offersToAnalyze.length < offerCount) {
          ...
          if (!res.ok) {
            ...
            try {
              ...
              throw new Error(errText.slice(0, 200) || 'Comparison failed');
            }
          }
          ...
          try {
            ...
            const validOffers = Array.isArray(result.offers) ? result.offers.filter((offer: CompareOffer) => offer?.data) : [];
            if (validOffers.length === 0) {
              const firstError = Array.isArray(result.offers)
                ? result.offers.find((offer: CompareOffer) => offer?.error)?.error
                : null;
              throw new Error(firstError || 'Comparison failed');
            }

            offerResults.push(...result.offers);

            // Use comparison from main response (already generated by API)
            setCompareResult({ address, offers: offerResults, comparison: result.comparison || null });
          } catch (err) {
            const msg = err instanceof DOMException && err.name === 'AbortError'
              ? 'Comparison timed out. Try fewer offers at once or retry in a moment.'
              : err instanceof Error
                ? err.message.slice(0, 200)
                : String(err).slice(0, 200);

            // If absolutely nothing parsed, surface a real error instead of a fake N/A comparison table.
            setError(msg);
            setCompareResult(null);
            return;
          }
